...
module.exports.getAllMeasurements = function(callback){
    var query = "SELECT measurement.*, experiment.Name as ExperimentName, experiment.Description as ExperimentDescription, subject.firstname as SubjectFirstName, subject.LastName as SubjectLastName, subject.Height as SubjectHeight, subject.Weight as SubjectWeight, subject.Age as SubjectAge, subject.BirthDay as SubjectBirthDay, subject.Remark as SubjectRemark FROM `measurement` inner join experiment on experiment.id = measurement.ExperimentID inner join subject on measurement.SubjectID=subject.ID ORDER BY MeasurementDate DESC";
